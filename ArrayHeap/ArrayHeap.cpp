...
public:
    ...
    priorityData removeMin(int i);
    priorityData returnMin(int i);
    std::string toString();
    void bubbleDown(int index);

private:
    priorityData heap[10];  // Array of priorityData as a class member
    int capacity;
    int size = 0;
};
...
ArrayHeap::priorityData ArrayHeap::removeMin(int i)
{
       if (size == 0) {
       ...
       for(int j = 0; j < capacity; j++)
       {
           ...
           {
               intfound = true;
               priorityData temp = heap[j];
               heap[j] = heap[size - 1];  // Replace the removed element with the last element in the heap
               size--;  // Decrease the size of the heap
               bubbleDown(j);  // Perform a bubble down operation to maintain the heap property
               std::cout<< "The value at " + std::to_string(j) + "was deleted!";
               return temp;
           }
           //bubble down
...
ArrayHeap::priorityData ArrayHeap::returnMin(int i)
{
       bool intfound = false;
       for(int j = 0; j < capacity; j++)
       {
